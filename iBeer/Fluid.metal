...
kernel void updateParticles(device Particle *p [[buffer(0)]], constant Uniforms &u [[buffer(1)]], uint id [[thread_position_in_grid]]) {
    ...
    if (abs(q.x) > 0.31) { q.x = clamp(q.x, -0.31, 0.31); q.vx *= -0.5; }
...
vertex VertexOut particleVertex(uint vertexID [[vertex_id]], uint instanceID [[instance_id]], device const Particle *p [[buffer(0)]], constant Uniforms &u [[buffer(1)]]) {
    ...
    Particle q = p[instanceID]; float2 c = corners[vertexID]; float2 pos = float2(q.x + c.x*q.size, q.y + c.y*q.size); float cs = cos(u.tilt), sn = sin(u.tilt); pos = float2(pos.x*cs-pos.y*sn, pos.x*sn+pos.y*cs); VertexOut o; o.position = float4(pos.x * 1.55, pos.y * 0.93, 0, 1); o.uv = c * 0.5 + 0.5; o.kind = q.kind; return o;
}
fragment float4 particleFragment(VertexOut in [[stage_in]], constant Uniforms &u [[buffer(1)]]) {
    ...
    return float4(u.color.rgb + 0.08, a * u.color.a * 0.80);
}
